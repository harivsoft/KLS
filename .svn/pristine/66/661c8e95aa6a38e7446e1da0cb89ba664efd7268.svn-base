...
public class LandRegisterActionServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(LandRegisterActionServlet.class);

	public LandRegisterActionServlet() {
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("Start: calling LandRegisterActionServlet() inside doPost() ");
		PrintWriter printWriter = null;
		logger.info("memberNo--" + request.getParameter("customerId"));
		Integer memberId = Integer.parseInt(request.getParameter("customerId"));
		...
		if(pdfRequired.equalsIgnoreCase("Y")){
			...
			fileName= new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
					+ ReportConfig.getProperty("LandRegister-Report-PDF-FileName"));	
		} else  {
			
			fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
				+ ReportConfig.getProperty("LandRegister-Report-Txt-FileName"));
		}
		...
		if(pdfRequired.equalsIgnoreCase("Y")){
			...
			session.setAttribute("outDatName", ReportConfig.getProperty("LandRegister-Report-Bat-FileName"));
			session.setAttribute("reportName", ReportConfig.getProperty("LandRegister-Report-Txt-FileName"));
		}
		...
		try {
			map = getLandRegisterIreport(requiredFormat, printWriter, memberId, fileName, pacsId, userName, businessDate);
		} catch (Exception e1) {
			e1.printStackTrace();
...
//		} 
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				...
				{		
					...
					response.setContentType("application/pdf");
					 ...
					 String filename=PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("LandRegister-Report-PDF-FileName");
	...
	public Map<String, Object> getLandRegisterIreport(String reportType, PrintWriter printWriter, Integer memberId, File filename, Integer pacsId,
			String userName, String businessDate) throws ParseException {
		logger.info("Start: Calling LandRegisterService in getLandRegisterIreport()");
		...
		if (pac != null) {
			parameters.put("bankName", pac.getBranch().getName());
		...
		try {
			...
			if (reportType.equals("pdf")) {
				...
				List<LandRegisterReportData> list = KLSReportSeriveFactory.getLandRegisterReportService().getLandRegisterDetails(memberId, pacsId);
				...
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
						.getProperty("LandRegister-Report-Jrxml")), new JRBeanCollectionDataSource(list), parameters);
			...
			if (reportType.equals("html")) {
				...
				if (list.isEmpty())
					return null;
				logger.info("LandRegisterList:" + list.toString());
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
						.getProperty("LandRegister-Report-Jrxml")), new JRBeanCollectionDataSource(list), parameters);
				...
				String footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("LandRegister-FPReport-JSP"), true, null);
			...
			e.printStackTrace();
			logger.error("Error:Inside getLandRegisterIreport method");
			throw new KlsReportRuntimeException("Problem occured while generating report");
		}
		logger.info("End: Calling LandRegisterService in getLandRegisterIreport()");
	...
	public static String callFastPrint(String txtFileName) {
		...
		String txtName=ReportUtil.writeToFile(ReportConfig.getProperty("LandRegister-Report-Bat-FileName"), printerCommand,
		txtFileName);
