 ...
 * 
 */
public class DrawalReportActionServlet extends HttpServlet {

	private static final long serialVersionUID = -2156179601256068005L;
	private static final Logger logger = Logger.getLogger(DrawalReportActionServlet.class);

	public DrawalReportActionServlet() {
	}

	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

		logger.info("Start: calling getDrawlIreport() inside doPost() ");
		...
		String error = "Error Occured While Report Generation ";
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				fileName= new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("Drawal-Report-PDF-FileName"));	
			} else  {
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
					+ ReportConfig.getProperty("Drawal-Report-Txt-FileName"));
			}
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				...
				session.setAttribute("txtFileName", fileName.getPath());
				 session.setAttribute("outDatName", ReportConfig.getProperty("Drawal-Report-Bat-FileName"));
				 session.setAttribute("reportName", ReportConfig.getProperty("Drawal-Report-Txt-FileName"));
			}
			fileName = ReportUtil.getFileName(fileName);
			HttpSession session = request.getSession();
			session.setAttribute("txtFileName", fileName.getPath());
			...
			try
			{
			map = getDrawlIreport(requiredFormat, fromDateString, toDateString, printWriter, schemeId, seasonId, fileName, pacsId, bankName, userName,
...
//			}
				...
				if(pdfRequired.equalsIgnoreCase("Y")){
					...
					{		
						...
						response.setContentType("application/pdf");
						 ...
						 String filename=PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("Drawal-Report-PDF-FileName");
	...
	public Map<String, Object> getDrawlIreport(String reportType, String fromDateString, String toDateString, PrintWriter printWriter,
			Integer schemeId, Long seasonId, File filename, String pacsId, String bankName, String userName, String businessDate)
			throws ParseException {
		logger.info("Start: Calling DrawalReportService in getDrawlIreport()");
		...
		try {
			...
			if (reportType.equals("pdf")) {
				...
				List<DrawalReportData> list = KLSReportSeriveFactory.getDrawalReportService().getDrawalReport(pacId, schemeId, seasonId, fromDate,
						toDate);
				...
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
						.getProperty("Drawal-Report-Jrxmll")), new JRBeanCollectionDataSource(list), parameters);
			...
			if (reportType.equals("html")) {
				...
				if (list.isEmpty())
					return null;
				logger.info("drawalList:" + list);
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(
						ReportUtil.class.getResourceAsStream(ReportConfig.getProperty("Drawal-Report-Jrxml")), new JRBeanCollectionDataSource(list),
						parameters);
				...
				list6.add(pac != null ? pac.getVillage().getName() : "");
				...
				String footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("Drawal-FPReport-JSP"), true, null);
			...
			logger.error("Error:Inside getDrawlIreport method");
			throw new KlsReportRuntimeException("Problem occured while generating report");
		}
		logger.info("End: Calling DrawalReportService in getDrawlIreport()");
	...
	public static String callFastPrint(String txtFileName) {
		...
		String txtName = ReportUtil.writeToFile(ReportConfig.getProperty("Drawal-Report-Bat-FileName"), printerCommand, txtFileName);
