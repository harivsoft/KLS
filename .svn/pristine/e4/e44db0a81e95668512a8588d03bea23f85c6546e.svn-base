 ...
 * 
 */
public class AccountStatementActionServlet extends HttpServlet {

	private static final long serialVersionUID = 8249361955406450693L;

	private static final Logger logger = Logger.getLogger(AccountStatementActionServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String includeInterest = request.getParameter("radIncludeInterestPart");
			String includeCharges = request.getParameter("radIncludeChargesPart");
			String includeParticulars = request.getParameter("radIncludeParticulars");
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				fileName= new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("AccStatement-Report-PDF-FileName"));	
			} else  {
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
					+ ReportConfig.getProperty("AccStatement-Report-Txt-FileName"));
			}
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				...
				session.setAttribute("txtFileName", fileName.getPath());
				 session.setAttribute("outDatName", ReportConfig.getProperty("AccStatement-Report-Bat-FileName"));
				 session.setAttribute("reportName", ReportConfig.getProperty("AccStatement-Report-Txt-FileName"));
			}
		
			fileName = ReportUtil.getFileName(fileName);
			HttpSession session = request.getSession();
			session.setAttribute("txtFileName", fileName.getPath());
			Map<String, Object> map = getAccountStatementReport(requiredFormat, fromDateString, toDateString, printWriter, accNo, includeInterest,
					includeCharges, includeParticulars, pacsId, bankName, fileName, userName, businessDate);
//			if (map != null) {
...
//			} 
				...
				if(pdfRequired.equalsIgnoreCase("Y")){
					...
					{		
						...
						response.setContentType("application/pdf");
						 ...
						 String filename=PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("AccStatement-Report-PDF-FileName");
	...
	public Map<String, Object> getAccountStatementReport(String reportType, String fromDateString, String toDateString, PrintWriter printWriter,
			String accNo, String includeInterest, String includeCharges, String includePerticulars, String pacsId, String bankName, File fileName,
			String userName, String businessDate) {
		...
		try {
			...
			Account account = KLSDataAccessFactory.getAccountDAO().getAccount(accNo);
			if (account == null)
				throw new AccountDoesntExistsException("Account does not exist");
			PersonData customer = null;
			customer = RestClientUtil.getCustomerById(account.getAccountProperty().getCustomerId());
			String shareBalance = RestClientUtil.getShareBalanceByCustomerId(customer.getCustomerId(), toDateString);
			List<AddressData> addressList = customer.getContactDetailList();
			AddressData addressData = null;
			Village village = new Village();
			if (!addressList.isEmpty()) {
				addressData = addressList.get(0);
				village.setId(addressData.getVillageId());
				village = KLSDataAccessFactory.getVillageDAO().getVillage(village);
			}
			parameters.put("pacName", pac != null ? pac.getName() : "");
			parameters.put("address", addressData.getLine1());
			parameters.put("villageName", village.getName());
			parameters.put("fromDate", fromDateString);
			parameters.put("toDate", toDateString);
			parameters.put("includeInterest", includeInterest);
			parameters.put("includeCharges", includeCharges);
			parameters.put("includePerticulars", includePerticulars);
			parameters.put("accNo", accNo);
			parameters.put("customerId", customer.getMemberNumber());
			parameters.put("custName", customer.getName());
			parameters.put("contactNo", addressData.getMobileNo());
			...
			if (reportType.equals("pdf")) {
				...
				List<AccountStatementReportData> list = KLSReportSeriveFactory.getAccountStatementReportService().getAccountStatementReport(accNo,
						fromDate, toDate);
				...
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
						.getProperty("AccStatement-Report-Jrxml")), new JRBeanCollectionDataSource(list), parameters);
			...
			if (reportType.equals("html")) {
				...
				AccountStatementReportData accountStatementReportData = list.get(0);

				parameters.put("openingBalance", accountStatementReportData.getPrincipalBalance());
				// list.remove(0);
				Iterator<AccountStatementReportData> iterator = list.iterator();

				while (iterator.hasNext()) {
					AccountStatementReportData data = (AccountStatementReportData) iterator.next();
				...
				list12.add(accountStatementReportData.getPrincipalBalance());
				...
				String footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("AccStatement-FPReport-JSP"), true, null);
	...
	public static String callFastPrint(String txtFileName) {
		...
		String txtName = ReportUtil.writeToFile(ReportConfig.getProperty("AccStatement-Report-Bat-FileName"), printerCommand, txtFileName);
