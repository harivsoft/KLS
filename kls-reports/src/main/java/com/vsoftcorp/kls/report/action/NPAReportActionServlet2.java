...
public class NPAReportActionServlet2 extends HttpServlet {
	
		/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
		private static final Logger logger = Logger.getLogger(NPAReportActionServlet2.class);
		...
		public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			logger.info("Start: calling NPAReportActionServlet2() inside doPost() ");
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				if("L".equals(loanType)) {
					fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
							+ ReportConfig.getProperty("NPA-Report-PDF-FileName1"));
				} else {
					fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
							+ ReportConfig.getProperty("NPA-Report-PDF-FileName1"));
				}
				...
				if("L".equals(loanType)) {
					fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
							+ ReportConfig.getProperty("NPA-Report-Txt-FileName1"));
				} else {
					fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
							+ ReportConfig.getProperty("NPA-Report-Txt-FileName1"));
				}
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				...
				session.setAttribute("outDatName", ReportConfig.getProperty("NPA-Report-Bat-FileName1"));
				session.setAttribute("reportName", ReportConfig.getProperty("NPA-Report-Txt-FileName1"));
			}
			...
			try {
				map = getNpaReport(requiredFormat, loanType, pacsId, asOnDate, reportType, fileName,businessDate,userName,bankName,excludeStndLoan,productId);
			} catch (Exception e1) {
				e1.printStackTrace();
			...
			if (map != null)  {
				...
				if(pdfRequired.equalsIgnoreCase("Y")){
					...
					{		
						...
						if("L".equals(loanType)) {
							filename =  PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("NPA-Report-PDF-FileName1");
						} else {
							filename = PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("NPA-Report-PDF-FileName1");
						}
		...
		public Map<String, Object> getNpaReport(String requiredFormat, String loanType, Integer pacsId, String asOnDate, String reportType, File filename, String businessDate,String userName,String bankName,String excludeStndLoan,Integer productId) throws ParseException {
			...
			try {
				...
				List<NPAReportData2> list = KLSReportSeriveFactory.getNPAReportService2().getNPAReportData( asOnDate, loanType, reportType,excludeStndLoan, productId,pacsId);
				if (list.isEmpty())
					return null;
				...
				if (requiredFormat.equals("pdf")) {
					...
					if("L".equals(loanType)) {
						jasperPrint =	ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
								.getProperty("NPA-Report-Jrxml1")), new JRBeanCollectionDataSource(list), parameters);
					} else {
						jasperPrint =	ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
								.getProperty("NPA-Report-Jrxml1")), new JRBeanCollectionDataSource(list), parameters);
					}
				...
				if (requiredFormat.equals("html")) {
					...
					if("L".equals(loanType)) {
						footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("npa-FPReport-JSP2"), true, null);
					} else {
						footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("npa-FPReport-JSP2"), true, null);
					}
		...
		public static String callFastPrint(String txtFileName) {
			logger.info("Start: inside callFastPrint() this is 6 parameter function.... ");
