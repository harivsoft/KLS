...
public class BorrowingAccountLedgerActionServlet extends HttpServlet {
	...
	public BorrowingAccountLedgerActionServlet() {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pdfRequired.equalsIgnoreCase("Y")){
			fileName= new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
					+ ReportConfig.getProperty("BorrowingAccountLedger-Report-PDF-FileName"));	
		} else  {
			
			fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
				+ ReportConfig.getProperty("BorrowingAccountLedger-Report-Txt-FileName"));
		}
		...
		if(pdfRequired.equalsIgnoreCase("Y")){
			...
			session.setAttribute("txtFileName", fileName.getPath());
			 session.setAttribute("outDatName", ReportConfig.getProperty("BorrowingAccountLedger-Report-Bat-FileName"));
			 session.setAttribute("reportName", ReportConfig.getProperty("BorrowingAccountLedger-Report-Txt-FileName"));
		}
		...
		try {
			map = getBorrowingAccountLedgerDataList(requiredFormat, pacsId, productId, purposeId, borrowingType, fromDateString, toDateString, printWriter,
					fileName, userName, businessDate, bankName,branchId);
		} catch (Exception e1) {
			e1.printStackTrace();
...
//			}
	...
	public Map<String, Object> getBorrowingAccountLedgerDataList(String reportType, Integer pacsId, Integer productId, Integer purposeId,
			...
			String businessDate, String bankName,Integer branchId) throws ParseException {
		...
		try {
			...
			if (reportType.equals("pdf")) {
				...
				List<BorrowingAccountLedgerReportData> list= KLSReportSeriveFactory
						.getBorrowingAccountLedgerReportService().getBorrowingAccountLedgerReportReport(pacsId, productId, purposeId, borrowingType,
								fromDateString, toDateString,branchId);
			...
			if (reportType.equals("html")) {
				...
				{
					getBorrowingAccountLedgerReportDataList.add(null);
				...
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(
						ReportUtil.class.getResourceAsStream(ReportConfig.getProperty("BorrowingAccountLedger-Report-Jrxm")),
				...
				String footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("BorrowingAccountLedger-FPReport-JSP"), true, null);
	...
	public static String callFastPrint(String txtFileName) {
		...
		String txtName=ReportUtil.writeToFile(ReportConfig.getProperty("BorrowingAccountLedger-Report-Bat-FileName"), printerCommand,
		txtFileName);
