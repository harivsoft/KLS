...
public class ScheduleReportActionServlet extends HttpServlet{
	private static final long serialVersionUID = 8249361955406450693L;

	private static final Logger logger = Logger.getLogger(ScheduleReportActionServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		logger.info("start: calling getScheduleReport() in doPost()");
		PrintWriter printWriter = null;
		String error = "Error Occured While Report Generation  ";
		try {
			String pacsId = request.getParameter("pacId");
			...
			logger.info("parameter values:pacsId::"+pacsId+"toDate::"+toDateString+"productId::"+productId);
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				fileName= new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("Schedule-Report-PDF-FileName"));	
			} else  {
				
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
					+ ReportConfig.getProperty("Schedule-Report-Txt-FileName"));
			}
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				...
				session.setAttribute("outDatName", ReportConfig.getProperty("Schedule-Report-Bat-FileName"));
				session.setAttribute("reportName", ReportConfig.getProperty("Schedule-Report-Txt-FileName"));
			}
			...
			HttpSession session = request.getSession();
			session.setAttribute("txtFileName", fileName.getPath());
			Map<String, Object> map = getScheduleReport(requiredFormat,  toDateString, printWriter, pacsId,fileName,bankName,userName,businessDate,productName,productId);
//			if (map != null) {
...
//			}
				...
				if(pdfRequired.equalsIgnoreCase("Y")){
					...
					{		
						...
						response.setContentType("application/pdf");
						 ...
						 String filename=PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("Schedule-Report-PDF-FileName");
	...
	public Map<String, Object> getScheduleReport(String reportType,  String toDateString, PrintWriter printWriter,String pacsId,File fileName,String bankName,String userName,String businessDate,String productName,String productId) {
		logger.info("Start: Calling ScheduleReportService in ScheduleReport()");
		try {
			Pacs pac = null;
			...
			//parameters.put("balanceType", balanceType);
...
logger.info("product id is:::"+productId);
			...
			if (reportType.equals("pdf")) {
				...
				List<ScheduleReportData> list = KLSReportSeriveFactory.getScheduleVsGLBalanceReportService().getScheduleBalance(pacsId,toDate,productId);
				...
				JasperPrint jasperPrint = ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
						.getProperty("Schedule-Report-Jrxml")), new JRBeanCollectionDataSource(list), parameters);
			...
			if (reportType.equals("html")) {
				...
				ScheduleReportData accountStatementReportData = list.get(0);

				//parameters.put("Loan Type", accountStatementReportData.getLoanType());
				// list.remove(0);
				Iterator<ScheduleReportData> iterator = list.iterator();
			...
			/*	List<String> list11 = new ArrayList<String>();
				...
				list12.add(accountStatementReportData.getClosingBalance());
				...
				String footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("Schedule-FPReport-JSP"), true, null);
	...
	public static String callFastPrint(String txtFileName) {
		...
		String txtName = ReportUtil.writeToFile(ReportConfig.getProperty("Schedule-Report-Bat-FileName"), printerCommand, txtFileName);
