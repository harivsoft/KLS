...
public class LoanRegisterSummeryReportActionServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("Start: calling LoanRegisterSummeryReportActionServlet() inside doPost() ");
		PrintWriter printWriter = null;
		String  strPacsId = request.getParameter("pacsId");
		Integer pacsId = null;
		if(strPacsId != null && !"null".equalsIgnoreCase(strPacsId) ){
		...
		if(pdfRequired.equalsIgnoreCase("Y")){
			if("L".equals(loanType)) {
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("LoanRegisterMtSummery-Report-PDF-FileName"));
			} else {
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("LoanRegisterStSummery-Report-PDF-FileName"));
			}
		} else {
			if("L".equals(loanType)) {
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("LoanRegisterMtSummery-Report-Txt-FileName"));
			} else {
				fileName = new File(PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport/"
						+ ReportConfig.getProperty("LoanRegisterStSummery-Report-Txt-FileName"));
			}
		}
		...
		if(pdfRequired.equalsIgnoreCase("Y")){
			...
			session.setAttribute("reportName", ReportConfig.getProperty("LoanRegisterMtSummery-Report-Txt-FileName"));
			session.setAttribute("reportName", ReportConfig.getProperty("LoanRegisterStSummery-Report-Txt-FileName"));
		...
		try {
			map = getLoanRegisterSummeryReport(requiredFormat, loanType, pacsId, businessDate, productId, fileName, productName, loginUser);
		} catch (Exception e1) {
			e1.printStackTrace();
		...
		if (map != null)  {
			...
			if(pdfRequired.equalsIgnoreCase("Y")){
				...
				{		
					...
					if("L".equals(loanType)) {
						filename =  PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("LoanRegisterMtSummery-Report-PDF-FileName");
					} else {
						filename = PropertiesUtil.getDocumentProperty("filesLocation") + "SBReport"+ ReportConfig.getProperty("LoanRegisterStSummery-Report-PDF-FileName");
					}
	...
	public Map<String, Object> getLoanRegisterSummeryReport(String requiredFormat, String loanType, Integer pacsId, String businessDate, Integer productId, File filename, String productName, String loginUser) throws ParseException {
		logger.info("Start: Calling AccountInfoSummeryReportService in getAccountInfoIreport()");
		...
		try {
			logger.info(">>>>>> "+pacsId+","+productId+","+loanType);
			List<LoanRegisterData> list = KLSReportSeriveFactory.getLoanRegisterReportService().getLoanRegisterSummeryReport(pacsId, productId, DateUtil.getVSoftDateByString(businessDate), loanType);
			...
			if (requiredFormat.equals("pdf")) {
				...
				if("L".equals(loanType)) {
					jasperPrint =	ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
							.getProperty("LoanRegisterMtSummery-Report-Jrxml")), new JRBeanCollectionDataSource(list), parameters);
				} else {
					jasperPrint =	ReportUtil.getJasperPrint(ReportUtil.class.getResourceAsStream(ReportConfig
							.getProperty("LoanRegisterStSummery-Report-Jrxml")), new JRBeanCollectionDataSource(list), parameters);
				}
			...
			if (requiredFormat.equals("html")) {
				...
				if("L".equals(loanType)) {
					footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("LoanRegisterMtSummery-FPReport-JSP"), true, null);
				} else {
					footerHtml = ReportUtil.getFastPrint(map, ReportConfig.getProperty("LoanRegisterStSummery-FPReport-JSP"), true, null);
				}
